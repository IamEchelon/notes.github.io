...
class Note():
    ...
    def __init__(self, color, shape, value, tone_val, path, hex_value):
        self.color = color
        self.shape = shape
        self.value = value
        self.tone_val = tone_val
        self.path = path
        self.hex_value = hex_value

    def __str__(self):
        return "Color: {}, Shape: {}, Midi Value: {}, ToneJS: {}".format(self.color, self.shape, self.value, self.tone_val)

def pitch_octave_notation():
    """Constructor func for building tonejs readable note values"""
    ...
    for octave in OCTAVES:
        for pitch in PITCHES:
            toneJS_vals.append(merge_notation(pitch, octave))

    return toneJS_vals


def merge_notation(pitch, octave):
    """Helper function used to stringify pitch-oct notation"""
    return "{}{}".format(pitch, octave)


def note_builder():
    """Note Constructor for entire Notes system"""
    note_container = []
    iterator = 0
    # constants
    TONEJS = pitch_octave_notation()
    COLORS = ["black", "brown", "red", "orange", "yellow", "green", "cyan", "blue", "purple", "pink", "grey" ]
    SHAPES = ["circle", "moon", "star", "plus", "triangle", "square", "key", "leaf", "crown", "cube", "minus", "sun"]
    ...
    HEX = ["#4A4A4A", "#A49085", "#E14A39", "#FB9F26", "#FBC51D", "#60BB6C", "#56ABD4", "#2B82C9", "#9464BA", "#ED6689", "#D8D8D8"]
    ...
    for index, color in enumerate(COLORS):
        for sindex, shape in enumerate(SHAPES):
            if iterator < 128:
                note_container.append(Note(color, shape, iterator, TONEJS[iterator], PATHS[sindex], HEX[index] ))
                iterator += 1
...
def to_json():
    ...
    with open('notesDB.json', 'w') as f:
        for note in NOTES:
            json.dump(note.__dict__, f, indent=2, separators=(',', ': ') )
